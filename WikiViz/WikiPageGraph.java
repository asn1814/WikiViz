...
import java.net.*; //just for the URL object, everything else is Jsoup 
...
import edu.uci.ics.jung.visualization.decorators.*; //for labelling the edges
import edu.uci.ics.jung.visualization.control.*; //for mouse listener
...
public class WikiPageGraph {
   ...
   //default values
   public static double minRelevance = 0.007;
...
/*to allow edges to exist that are the same frequency but have different endpoints.
JUNG doesn't work with any edges that have the same value, so the wrapper ensures 
edges of the same value have different hashcodes*/
class Wrapper {
   int frequency;
