...
import org.jsoup.nodes.Document; // for jSoup Document, object representing a webpage
import org.jsoup.nodes.Element; // for HTML Element
import org.jsoup.select.Elements; // for a special list of HTML elements
...
public class WikiPage {
   ...
   /*the method which allows branches off this article
   Returns a hashmap where the keys are other articles, 
   values are how relevant they are to this article
   */
   public HashMap <WikiPage, Integer> getRelated (double minRelevance, int maxRelated) throws Exception {
      ...
      this.title = tempTitle.substring(0, tempTitle.length() - 12);
      Document doc = getGoodDoc(d);
      
      
      // get raw text, for searching 
      String allText = d.text().toLowerCase();
      ...
      //make the relevance factor independent of article size
      for (WikiPage w : absFreq.keySet()) {
   ...
   //shaves off references, citations, keeps content of the article
   ...
   //populates the keys of the hashmap, doesn't count the occurrences
